...
'''
This program is an implementation of the greedy nearest-neighbor's heuristic for the Traveling Salesman Problem with Appointments.
'''
...
#Returns the closest point to a given coordiante.
def closestpoint(point, locations):
    dmin = float("inf")
    for p in locations:
        d = distance(point, p)
...
def main():
    ...
    for vehicle in range(len(start_locations)): #Each "vehicle" is the time between each of my appointments.
        ...
        while path[-1] != end_locations[vehicle] or len(path) < 2:
            ...
            remaining = distance(path[-1], end_locations[vehicle]) #How much time I have need to get to the next appointment in time.
            ...
            closest, dist = closestpoint(path[-1], locations)
            returndist = distance(closest, end_locations[vehicle])
            # print (onesum)
            # print (dist)
            # print (remaining)
    ...
    printMap(allpath)
    print ("Time of last action: ", round(allsum))
